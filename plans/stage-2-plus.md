...
- Style: default, base colour: slate, CSS variables enabled
- Components live in `frontend/components/ui/` — manually written, NOT generated by CLI
- Add new components by copying the pattern from existing ones (`button.tsx`, `card.tsx`, etc.)
- All `@radix-ui/*` primitives are already in `package.json` — no new installs needed for standard shadcn components
- Charts use Recharts (`recharts` package) wrapped in `frontend/components/charts/`
- Data fetching uses TanStack Query (`@tanstack/react-query`) — wrap mutations/queries with `useMutation`/`useQuery`, invalidate on success
